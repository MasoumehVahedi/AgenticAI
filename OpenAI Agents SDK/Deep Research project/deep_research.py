...
from clarifier_agent import ClarifyingQuestions, clarifier_agent
...
async def submit_answer(answer, answers_list, questions_list, current_index, original_query):
    if not answer or len(answer.strip()) > 100:
        return gr.update(value="Answer must be 1-100 characters."), answers_list, current_index, gr.update(
    ...
    if next_index < len(questions_list):
        ...
        return "", answers_list, next_index, gr.update(value=formatted), gr.update(visible=True), gr.update(
            visible=False), gr.update(visible=False), ""
    else:
        contextualized_query = await run_contextualizer(original_query, questions_list, answers_list)
        return gr.update(visible=False), answers_list, current_index, gr.update(
            value=f"**Contextualized Query:** {contextualized_query}"), gr.update(visible=False), gr.update(
            visible=True), gr.update(visible=True), contextualized_query
...
async def run_contextualizer(original_query, questions_list, answers_list):
    ...
    for idx, (q, a) in enumerate(zip(questions_list, answers_list), 1):
        formatted_input += f"Clarifying Question {idx}:\n{q.clarifying_question.strip()}\nPurpose of clarifying question {idx}:\n{q.question_purpose.strip()}\nUser Answer for Question {idx}:\n{a.strip()}\n\n"
    result = await Runner.run(contextualizing_agent, formatted_input)
    return result.final_output_as(ContextualizedQuestions).contextualized_query


async def get_clarifying_questions(query: str):
    query = query.strip()
    if not query or len(query) < 5 or len(query) > 100:
        return "Please provide a more specific query (5–100 characters).", [], gr.update(visible=False), gr.update(
            visible=False), gr.update(visible=False)
...
with gr.Blocks(theme=gr.themes.Default(primary_hue="sky")) as ui:
    ...
    submit_button.click(
        ...
        outputs=[answer_box, answers_state, question_index, question_display, submit_button, research_button,
                 research_report, contextualized_query_state]
    )
